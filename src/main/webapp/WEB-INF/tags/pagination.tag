...
<input type="button" class="btn" value="&lt;&lt;" onclick="$('#page').val('1');this.form.submit();" style="width:40px;"<c:if test="${pagedList.firstPage}"> disabled="disabeld"</c:if>/>
<input type="button" class="btn" value="&lt;" onclick="$('#page').val('${pagedList.number}');this.form.submit();" style="width:40px;"<c:if test="${pagedList.firstPage}"> disabled="disabeld"</c:if>/>
<input type="text" id="page" name="page" value="${pagedList.number+1}" style="width:40px;text-align:right;"/> / ${pagedList.totalPages}
<input type="button" class="btn" value="&gt;" onclick="$('#page').val('${pagedList.number+2}');this.form.submit();" style="width:40px;"<c:if test="${pagedList.lastPage}"> disabled="disabeld"</c:if>/>
<input type="button"  class="btn" value="&gt;&gt;" onclick="$('#page').val('${pagedList.totalPages}');this.form.submit();" style="width:40px;"<c:if test="${pagedList.lastPage}"> disabled="disabeld"</c:if>/>
